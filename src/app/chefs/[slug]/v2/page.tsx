...
export async function generateMetadata({ params }: ChefPageProps): Promise<Metadata> {
  ...
  return {
    title: `${chef.name} - ${showInfo}${resultInfo} | Cheft (V2 Layout)`,
...
function getShowAbbreviation(showName: string): string {
  ...
  return abbrevs[showName] || showName.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 3);
}

export default async function ChefPageV2({ params }: ChefPageProps) {
  const { slug } = await params;
    ...
    <>
      ...
      <div className="min-h-screen overflow-auto" style={{ background: 'var(--bg-primary)', paddingTop: '64px' }}>
        ...
        <main>
          ...
          {/* TV Appearances - Horizontal Strip */}
          {sortedShows.length > 0 && (
            <div 
            ...
            >
              <div className="max-w-6xl mx-auto px-4">
                <div className="flex items-center gap-4 min-w-max">
                  <span className="font-mono text-[10px] tracking-widest flex-shrink-0" style={{ color: 'var(--text-muted)' }}>
                    TV CREDITS
                  </span>
                  ...
                  {sortedShows.map((appearance, idx) => (
                    <Link
                      ...
                      href={appearance.show?.slug && appearance.season 
                        ...
                        : '#'}
                      className="flex items-center gap-2 px-3 py-2 transition-colors hover:bg-black/5 rounded"
                    >
                      <span 
                        className="font-mono text-sm font-bold sm:hidden"
                        style={{ color: 'var(--accent-primary)' }}
                      >
                        {getShowAbbreviation(appearance.show?.name || '')}
                      </span>
                      <span 
                        className="font-mono text-sm font-bold hidden sm:inline"
                        style={{ color: 'var(--accent-primary)' }}
                      >
          ...
          {featuredRestaurant && (
            <section className="py-16" style={{ background: 'var(--bg-primary)' }}>
              <div className="max-w-6xl mx-auto px-4">
                ...
                {/* Other Restaurants - Horizontal Cards with Photo on Left */}
                          ...
                          <div className="flex-1 p-4 flex flex-col justify-center">
                            <div className="flex justify-between items-start gap-3">
                              <div className="min-w-0 flex-1">
                                <h3 
                                  className="font-display text-base font-bold truncate transition-colors group-hover:text-[var(--accent-primary)]"
                                  style={{ color: 'var(--text-primary)' }}
                                >
                                  {restaurant.name}
                                </h3>
